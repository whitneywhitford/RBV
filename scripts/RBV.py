...
def empirical_pvalue(window_het_snps, vcf_outfile, num_window_samples):
	...
	return sample_het_snps,cum_sum_pvalue
...
def random_readbal(no_of_samples,vcf_file,gaps,qualCutoff):
	...
	random_readbal = read_bal(variant_lines,qualCutoff)
...
def main(ref_file, gapfile, CNV_file, variant_permutations, vcf_file, qualCutoff, window_permutations, windows_size, out_file):

	ref_fai = str(ref_file) + ".fai"	#check if all fai are just fasta.fai/fa.fai
	
	#if HC needed

	#bgzip and tabix via pysam
	
	
	out = open(out_file, 'w')
	print >>out, "# RBV: Read balance validator"
	print >>out, "# " + str(sys.arg)
	...
	gap_sites = gaps(gapfile,CNVfile)
	rand_readbal = random_readbal(variant_permutations,vcf_file,gaps,qualCutoff)
	...
	for i in range(len(CNVs)):
		CNV_chr,CNV_start,CNV_stop,CNV_type=CNVs[i].strip().split()
		
		out.write(str(CNV_chr) + "\t" + str(CNV_start) + "\t" + str(CNV_stop) + "\t")
		
		#DEL
		CNV_vcf_file = "CNV_vcf_file_" + str(i) + ".vcf"
		vcf_fetch(vcf_file,CNV_vcf_file, chr, start, stop)
		...
		for w in range(len(random_windows)):
			window_count,window_chr,window_start,window_end = window.split()
			window_vcf_file = "rand_vcf_file_" + str(i) + "_" + str(w) + ".vcf"
			vcf_fetch(vcf_file,window_vcf_file, window_chr, window_start, window_end)
		...
		CNV_ttest_pvalue = stats.ttest_ind(CNV_readbal_array, rand_readbal_array, equal_var=False)
		CNV_kstest_pvalue = stats.ks_2samp(CNV_readbal_array, rand_readbal_array)
